<?php
	...
	if (is_admin() && !defined('DOING_AJAX')) {
		...
		if (file_exists($license_file) && current_user_can('manage_options')) {
			...
			if (!$deleted) {
				$GLOBALS['readmedel'] = sprintf(__('Failed to delete files: license.txt and readme.html from the %s folder. Remove them manually!', 'theme_text_domain'), ABSPATH);
			} else {
				$GLOBALS['readmedel'] = sprintf(__('Files: license.txt and readme.html Removed from the %s folder', 'theme_text_domain'), ABSPATH);
			}
	...
	{
		return __('Error: You entered an incorrect username or password.', 'theme_text_domain');
	}
